...
@login_required
def new_ticket():
    status_data = [('1', 'Unassigned')]
    agent_list = [('1', 'Unassigned')]
    form = TicketForm()
    form.agent_id.choices = agent_list
    form.status.choices = status_data
    if form.validate_on_submit():
        ticket = Ticket(title = form.title.data, status = 'Unassigned', agent_id = 'Unassigned' ,content = form.content.data, author = current_user)
...
def assign_ticket(ticket_id):
    ...
    if current_user.userrole != 'Admin':
        abort(403)
    status_data = [('Unassigned', 'Unassigned'), ('Assigned', 'Assigned'), ('Under_review', 'Under_review'), ('Complete', 'Complete')]
...
def ticket_status_email(ticket, user):
    ...
    html_content='<p>The status of your ticket has been updated to<br>{}<br>This is an automated messeage. For any queries contact our customer support</p>'.format(ticket.status))
    try:
        sg = SendGridAPIClient("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")
